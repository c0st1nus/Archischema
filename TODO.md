...
**Детали:**
- [x] 2.1.1. Исследовать механизм подключения к LiveShare сессии
- [x] 2.1.2. Определить, где происходит мерж локального и удаленного состояния
- [x] 2.1.3. Реализовать очистку локальных таблиц перед подключением к сессии
- [x] 2.1.4. Добавить предупреждение пользователю о потере локальных данных (если они не сохранены, и сохранить данные невозможно)
- [x] 2.1.5. Протестировать сценарий: создание локальных таблиц → подключение к сессии

**Реализованные изменения:**
- Изменена функция `apply_graph_state()` в `src/ui/canvas.rs` для полной замены локального графа состоянием из сессии
- Добавлен флаг `initial_sync_done` в `LiveShareContext` для предотвращения повторной синхронизации
- Интегрирован `NotificationManager` в `SchemaCanvas` для отображения предупреждения пользователю
- Исправлено предупреждение реактивного контекста в `NotificationManager::notify()`
...
**Реализовано:**
- [x] 2.2.1. Аудит текущей логики синхронизации таблиц
- [x] 2.2.2. Определить причины несинхронизации (конфликты, race conditions, т.д.)
  - Проблема: node_id (индекс узла) не является стабильным идентификатором между клиентами
  - При создании таблиц на разных клиентах индексы могут не совпадать
- [x] 2.2.3. Реализовать механизм принудительной синхронизации схем
  - Добавлен UUID в структуру TableNode для стабильной идентификации
  - UUID передается во всех GraphOperation для таблиц
  - При применении операций поиск таблицы выполняется сначала по UUID, затем по node_id
- [x] 2.2.4. Добавить синхронизацию при каждом изменении схемы
  - Все операции с таблицами теперь используют UUID для идентификации
- [x] 2.2.5. Реализовать синхронизацию при подключении нового пользователя к сессии
  - apply_graph_state сохраняет UUID из snapshot
  - create_graph_snapshot включает UUID в TableSnapshot
- [x] 2.2.6. Добавить механизм разрешения конфликтов (например, last-write-wins или timestamp-based)
  - UUID обеспечивает однозначную идентификацию таблиц
  - Fallback на node_id для обратной совместимости
- [x] 2.2.7. Обновить все GraphOperation для работы с UUID
- [x] 2.2.8. Обновить тесты для новой структуры данных
- [x] 2.2.9. Проверить компиляцию и прохождение тестов

#### 2.3. Проблема с синхронизацией метаданных диаграммы при подключении к LiveShare
**Подзадача:** При подключении к LiveShare сессии с паролем, имя диаграммы не синхронизируется с сервера

**Статус:** В РАБОТЕ

**Описание проблемы:**
- При подключении гостя к LiveShare сессии, созданной на основе сохраненной диаграммы, имя диаграммы не обновляется у гостя
- Протокол поддерживает передачу `diagram_id` и `diagram_name` в `RoomResponse`
- Сервер пытается получить имя диаграммы из `DiagramRepository` при аутентификации
- Effect в `EditorPage` должен обновлять `diagram_name` при получении `room_info`
- Добавлено логирование для диагностики проблемы

**Детали технической реализации:**
- Сервер (`src/core/liveshare/websocket.rs`): Получает diagram_name из DiagramRepository и включает в RoomResponse
- Клиент (`src/ui/liveshare_client.rs`): Получает RoomResponse и устанавливает room_info сигнал
- UI (`src/ui/pages/editor.rs`): Effect отслеживает изменения room_info и обновляет diagram_name

**Шаги для решения:**
- [ ] 2.3.1. Проверить, отправляется ли diagram_name с сервера (проверить логи браузера)
- [ ] 2.3.2. Проверить, устанавливается ли room_info сигнал корректно (проверить логи "Setting room_info signal")
- [ ] 2.3.3. Проверить, срабатывает ли Effect в EditorPage (проверить логи "LiveShare room_info updated")
- [ ] 2.3.4. Определить, на каком этапе теряется информация
- [ ] 2.3.5. Исправить проблему в зависимости от результатов диагностики
- [ ] 2.3.6. Убрать debug-логирование после решения проблемы

**Примечания:**
- Добавлено подробное логирование во всех ключевых точках для диагностики
- Исправлены критические ошибки жизненного цикла event listeners (disposed signals, dropped closures)
- Проблема может быть как на стороне сервера (не получает/не отправляет), так и на клиенте (не применяет)
...
**Решение:** Добавлен `pl-10` (padding-left: 40px) к input полю поиска в `src/ui/sidebar.rs`, чтобы текст и placeholder не накладывались на иконку поиска.

**Подзадачи:**
- [x] 3.1. Найти компонент поиска таблиц в коде
- [x] 3.2. Проверить CSS стили для иконок и placeholder
- [x] 3.3. Исправить padding/margin для правильного расположения иконки
- [x] 3.4. Убедиться, что иконка не перекрывает текст ввода
- [x] 3.5. Протестировать на разных размерах экрана

### 4. ✅ Иконка сохранения исходного кода
**Проблема:** Иконка сохранения source кода не подходит (не соответствует действию)

**Подзадачи:**
- [x] 4.1. Найти текущую иконку сохранения в Source Code режиме
- [x] 4.2. Выбрать более подходящую иконку (например, floppy disk, check mark, download)
- [x] 4.3. Заменить иконку в коде
- [x] 4.4. Убедиться, что новая иконка визуально понятна
- [x] 4.5. Проверить accessibility (alt text, aria-label)

**Решение:** Заменили иконку `ARROW_DOWN_TO_LINE` на `SAVE` (floppy disk) в `src/ui/source_editor.rs`. Иконка дискеты является универсальным символом сохранения и более понятна пользователям.

### 5. ✅ Миграция диалогов создания на HTML `<dialog>`
**Проблема:** Диалоги создания нужно вывести в HTML тег `<dialog>`

**Подзадачи:**
- [x] 5.1. Найти все диалоги создания (Create Table, Create Column, и т.д.)
- [x] 5.2. Изучить текущую реализацию диалогов (вероятно custom modal компоненты)
- [x] 5.3. Создать базовый компонент на основе `<dialog>` элемента
- [x] 5.4. Реализовать методы `.showModal()` и `.close()`
- [x] 5.5. Добавить поддержку backdrop стилей через `::backdrop` pseudo-element
- [x] 5.6. Мигрировать диалог создания таблицы
- [x] 5.7. Мигрировать диалог редактирования таблицы
- [x] 5.8. Улучшить UI/UX диалогов (spacing, margins, visual hierarchy)
- [x] 5.9. Убедиться в правильной работе Esc key для закрытия
- [x] 5.10. Проверить accessibility (focus management, ARIA)

**Реализовано:**
- Создан компонент `Dialog` на основе HTML `<dialog>` элемента с поддержкой `.showModal()` и `.close()`
- Создан компонент `DialogWithHeader` для диалогов с заголовком
- Мигрирован `NewTableDialog` на использование HTML `<dialog>` с улучшенным UI
- Мигрирован `TableEditor` на использование HTML `<dialog>` как модальное окно
- Улучшен UI/UX диалогов:
  - Увеличены отступы (padding) для лучшей читаемости
  - Улучшена визуальная иерархия с более четкими заголовками
  - Добавлены информационные блоки с иконками
  - Улучшены стили кнопок с градиентами и эффектами hover
  - Более четкое разделение секций с помощью borders
- Добавлена поддержка backdrop стилей через `::backdrop` pseudo-element
- Реализовано закрытие по Escape и клику на backdrop
- Автоматический сброс состояния формы при открытии диалога
- `ColumnEditor` оставлен как inline компонент в sidebar (не требует модального окна)
