...
- [ ] 1.24. Реализовать стратегию разрешения конфликтов (last-write-wins на основе version)
- [ ] 1.25. Добавить timestamp-based conflict resolution для одинаковых версий
- [ ] 1.26. Обработка удаленных элементов (soft delete с tombstones)
- [ ] 1.27. Тестирование reconciliation при одновременном редактировании

#### Фаза 7: Периодические snapshots
- [ ] 1.28. Создать фоновую задачу для сохранения snapshot каждые 20-30 секунд
- [ ] 1.29. Реализовать сериализацию SchemaGraph в BYTEA для PostgreSQL
- [ ] 1.30. Добавить логику восстановления из snapshot при подключении
- [ ] 1.31. Реализовать fallback на Firebase/S3 для больших snapshot (опционально)
- [ ] 1.32. Добавить очистку старых snapshots (keep last 10)

#### Фаза 8: Проверка прав доступа и безопасность
- [ ] 1.33. Интегрировать LiveShare с системой прав доступа к диаграммам
- [ ] 1.34. Проверка прав при создании сессии (только owner/editor)
- [ ] 1.35. Проверка прав при подключении к сессии (viewer+)
- [ ] 1.36. Добавить связь room_id ↔ diagram_id в таблице sessions
- [ ] 1.37. Реализовать автоматическое закрытие сессии при удалении диаграммы
- [ ] 1.38. Добавить rate limiting для предотвращения DDoS

#### Фаза 9: Исправление ссылки "Поделиться"
- [ ] 1.39. Изменить формат ссылки с `/?room=uuid` на `/editor/{diagram_id}?room=uuid`
- [ ] 1.40. Обновить функцию генерации ссылки в `settings_modal.rs` (copy_link)
- [ ] 1.41. Обновить отображение ссылки в UI (input field с room link)
- [ ] 1.42. Обновить функцию в `liveshare_panel.rs` (если используется)
- [ ] 1.43. Обработка параметра `?room=uuid` в EditorPage при загрузке
- [ ] 1.44. Автоматическое подключение к сессии при переходе по ссылке

#### Фаза 10: UI индикаторы и UX
- [ ] 1.45. Добавить индикатор статуса синхронизации (Syncing/Synced/Error)
- [ ] 1.46. Показывать количество активных пользователей в сессии
- [ ] 1.47. Добавить индикатор "кто сейчас редактирует" (цветные аватары)
- [ ] 1.48. Реализовать тултипы с именами пользователей при наведении на курсоры
- [ ] 1.49. Добавить уведомление при потере соединения с восстановлением
- [ ] 1.50. Показывать индикатор сохранения snapshot в БД

#### Фаза 11: Idle detection
- [ ] 1.51. Реализовать детектор активности пользователя (mousemove, keypress)
- [ ] 1.52. Добавить таймауты: IDLE (30 сек бездействия), AWAY (скрыта вкладка)
- [ ] 1.53. Транслировать idle статус другим пользователям
- [ ] 1.54. Визуально отличать активных/idle/away пользователей
- [ ] 1.55. Автоотключение от сессии после 10 минут AWAY (опционально)

#### Фаза 12: Тестирование и оптимизация
- [ ] 1.56. Нагрузочное тестирование с 10 пользователями
- [ ] 1.57. Нагрузочное тестирование с 50 пользователями
- [ ] 1.58. Нагрузочное тестирование с 100 пользователями
- [ ] 1.59. Измерить bandwidth usage и latency
- [ ] 1.60. Профилирование CPU и memory usage на сервере
- [ ] 1.61. Оптимизация горячих путей (если найдены bottlenecks)
- [ ] 1.62. Тестирование сценариев с плохим интернетом (packet loss, high latency)

#### Фаза 13: Документация и мониторинг
- [ ] 1.63. Добавить логирование всех критических операций
- [ ] 1.64. Добавить метрики (Prometheus/Grafana): активные сессии, пользователи, bandwidth
- [ ] 1.65. Создать документацию по архитектуре LiveShare
- [ ] 1.66. Документировать протокол WebSocket сообщений
- [ ] 1.67. Создать troubleshooting guide для типичных проблем
